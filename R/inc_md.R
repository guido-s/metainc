...
#' @param t2 ...
#' @param t3 ...
#' @param x R object of class \code{inc_md} or  \code{summary.inc_md}.
#' @param object R object of class \code{inc_md}.
...
inc_md <- function(sims, t, t1, t2, t3) {
  ...
  output <- list(ASI = round(ds,3), DI = round(di,3), class_distribution = cl2[,1:2],prop_over_null = propnull, stud_class=sim_studd, call = match.call())
  class(output) <- "inc_md"
  output
  }}
}
...
#' @rdname inc_md
#' @method summary inc_md
#' @export 

summary.inc_md <- function(object, ...) {
  output <- object
  class(output) <- "summary.inc_md"
  output
}
...
#' @rdname inc_md
#' @keywords print
#' @method print summary.inc_md
#' @export 

print.summary.inc_md <- function (x,...) {
  cat("Decision Inconsistency index (DI): ", round(x$DI,1), "%")
  cat("\n")
  cat("Across-Studies Inconsistency (ASI): ", round(x$ASI,1), "%")
  cat("\n")
